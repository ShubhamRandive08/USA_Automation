...
export async function lockedUser(page,validateComponent) {
    loginpage = new LoginPage(page)
    await loginpage.adminLogin(AdminLoginData.locked_user.username, AdminLoginData.locked_user.password, validateComponent)
}

// Methods for admin login senario

export async function  loginValidAdmin(page,validComponent) {
    loginpage = new LoginPage(page)
    await loginpage.adminLogin(AdminLoginData.perfect_admin.username,AdminLoginData.perfect_admin.password,validComponent)
}

export async function loginWithWrongUsernameAdmin(page,validateComponent) {
    loginpage = new LoginPage(page)
    await loginpage.adminLogin(AdminLoginData.worng_with_username.username,AdminLoginData.worng_with_username.password,validateComponent)
}

export async function loginWithWrongPasswordAdmin(page,validateComponent) {
    loginpage = new LoginPage(page)
    await loginpage.adminLogin(AdminLoginData.wrong_with_pass.username,AdminLoginData.wrong_with_pass.password,validateComponent)
}

export async function noUsernamePasswordAdmin(page,validateComponent) {
    loginpage = new LoginPage(page)
    await loginpage.adminLogin(AdminLoginData.assert_with_empty_field.username,AdminLoginData.assert_with_empty_field.password,validateComponent)
}